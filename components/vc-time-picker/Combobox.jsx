...
const Combobox = {
  ...
  methods: {
    onItemChange(type, itemValue, isNextDay) {
    ...
    getHourSelect(hour, isNextDay) {
      const { prefixCls, hourOptions, disabledHours, showHour, use12Hours, allowNextDay } = this;
      if (!showHour) {
        return null;
      }
      ...
      const selectedIndex = isNextDay
        ? hourOptionsAdj.indexOf(hourAdj) + 24
        : hourOptionsAdj.indexOf(hourAdj);
      const options = allowNextDay
        ? hourOptionsAdj
            .map(option => {
              return { ...formatOption(option, disabledOptions), isNextDay: false };
            })
            .concat(
              hourOptionsAdj.map(option => {
                return { ...formatOption(option, disabledOptions), isNextDay: true };
              }),
            )
        : hourOptionsAdj.map(option => formatOption(option, disabledOptions));
  ...
  render() {
    ...
    return (
      <div class={`${prefixCls}-combobox`}>
        {this.getHourSelect(value.hour(), value.isNextDay || false)}
